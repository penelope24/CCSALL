...
public class CFGNodeSimplifier {
     ...
     */
    private final Set<String> packageToAllType;
    private final List<String> imports;
    private final Set<DFVarNode> allFields;
    ...
    public void simplifyCFGNodeStr(Node node) {
        if (node instanceof MethodDeclaration) {
            ...
            if (parentNode.isPresent()) {
                ...
                for (String pt : packageToAllType) {
    ...
    private Set<DFVarNode> simplifyNode(Node node, Set<DFVarNode> parentDefVarMap) {
        ...
        if (node instanceof ExpressionStmt) {
            ...
            for (SwitchEntry caseEntry : caseEntries) {
                String caseLabel = caseEntry.getLabels().getFirst().isPresent() ? "case " + caseEntry.getLabels().getFirst().get().toString() : "default";
                int caseLineNum = caseEntry.getBegin().isPresent() ? caseEntry.getBegin().get().line : -1;
                GraphNode caseNode = allCFGNodesMap.get(caseLabel + ":" + caseLineNum);
                if (caseEntry.getLabels().getFirst().isPresent()) {
                    dealSingleRoadStmtDFG(copy, caseEntry.getLabels().getFirst().get(), caseNode);
...
//            }
    ...
    private String analysisExprForVar(Expression expression, Set<DFVarNode> parentDefVarMap, GraphNode node) {
        if (expression instanceof ArrayAccessExpr) {
            ...
            for (DFVarNode next : parentDefVarMap) {
            ...
            for (VariableDeclarator var : variables) {
                //先把之前的相同名字变量定义去除
                parentDefVarMap.removeIf(dfVarNode -> dfVarNode.getVarName().equals(var.getNameAsString()));
                //数据流更新
