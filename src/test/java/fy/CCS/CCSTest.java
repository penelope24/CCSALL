...
public class CCSTest {
    final String project = "/Users/fy/Documents/MyProjects/slicing_cases";
    ...
    public MethodDeclaration analyze_basic_case(int idx, int start) {
        ...
        MethodDeclaration n = Objects.requireNonNull(JPHelper.getCompilationUnit(javaFile)).findFirst(MethodDeclaration.class).get();
        MethodDeclaration nClone = n.clone();
        MethodPDG graph = PDGBuilder.one_pass_parse(project, javaFile, n);
        Set<Integer> reservedLines = DTEntry.dependencyTrack(Objects.requireNonNull(graph), start, MAX_DATA_DEPTH, MAX_CTRL_DEPTH);
        return SrcCodeTransformer.slice(nClone, reservedLines);
    }
    ...
    void test_advance_anonymous() {
        ...
        int start = 16;
        MethodDeclaration n = Objects.requireNonNull(JPHelper.getCompilationUnit(javaFile)).findAll(MethodDeclaration.class).get(1);
        MethodDeclaration nClone = n.clone();
        MethodPDG graph = PDGBuilder.one_pass_parse(project, javaFile, n);
        Set<Integer> reservedLines = DTEntry.dependencyTrack(Objects.requireNonNull(graph), start, MAX_DATA_DEPTH, MAX_CTRL_DEPTH);
        MethodDeclaration slice = SrcCodeTransformer.slice(nClone, reservedLines);
    ...
    void test_find_par_block() {
        ...
        SrcCodeTransformer.print(Objects.requireNonNull(parBlock));
    }
    ...
    void test_find_containing_stmt() {
        ...
        SrcCodeTransformer.print(Objects.requireNonNull(stmt));
    }
    ...
    void test_node_equality() {
        ...
        n.getParentNode().ifPresent(slice::setParentNode);
        MethodPDG g = PDGBuilder.one_pass_parse(project, javaFile, slice);
        Objects.requireNonNull(g).copyVertexSet().forEach(v -> {
    ...
    void bug1() throws GitAPIException {
        ...
        MethodDeclaration n = Objects.requireNonNull(cu).findAll(MethodDeclaration.class).stream()
...
//        System.out.println(n.getRange().get());
        ...
        MethodDeclaration clone = Objects.requireNonNull(n).clone();
